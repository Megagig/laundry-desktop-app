...
export function registerExpenseHandlers() {
    ipcMain.handle("expense:create", async (_event, sessionToken, data) => {
        try {
            ...
            await auditService.logDataOperation('CREATE', 'EXPENSE', result.id, permissionCheck.userId, permissionCheck.user.username, {
                title: result.title,
    ...
    ipcMain.handle("expense:getAll", async (_event, sessionToken, limit) => {
        try {
    ...
    ipcMain.handle("expense:getById", async (_event, sessionToken, id) => {
        try {
            const permissionCheck = await checkPermission(sessionToken, PERMISSIONS.VIEW_EXPENSE);
            if (!permissionCheck.success) {
                return { success: false, error: permissionCheck.error };
    ...
    ipcMain.handle("expense:getByDateRange", async (_event, sessionToken, startDate, endDate) => {
        try {
            const permissionCheck = await checkPermission(sessionToken, PERMISSIONS.VIEW_EXPENSE);
            if (!permissionCheck.success) {
                return { success: false, error: permissionCheck.error };
    ...
    ipcMain.handle("expense:getByCategory", async (_event, sessionToken, category) => {
        try {
            const permissionCheck = await checkPermission(sessionToken, PERMISSIONS.VIEW_EXPENSE);
            if (!permissionCheck.success) {
                return { success: false, error: permissionCheck.error };
    ...
    ipcMain.handle("expense:update", async (_event, sessionToken, data) => {
        try {
            ...
            // Log expense update
            if (result) {
                await auditService.logDataOperation('UPDATE', 'EXPENSE', result.id, permissionCheck.userId, permissionCheck.user.username, {
    ...
    ipcMain.handle("expense:delete", async (_event, sessionToken, id) => {
        try {
            ...
            if (expense) {
                await auditService.logDataOperation('DELETE', 'EXPENSE', id, permissionCheck.userId, permissionCheck.user.username, {
                    title: expense.title,
    ...
    ipcMain.handle("expense:getGroupedByCategory", async (_event, sessionToken, startDate, endDate) => {
        try {
            const permissionCheck = await checkPermission(sessionToken, PERMISSIONS.VIEW_EXPENSE);
            if (!permissionCheck.success) {
                return { success: false, error: permissionCheck.error };
