...
export function registerServiceHandlers() {
  ipcMain.handle("service:create", async (_event, sessionToken: string, data: CreateServiceDTO) => {
    try {
  ...
  ipcMain.handle("service:getAll", async (_event, sessionToken: string) => {
    try {
  ...
  ipcMain.handle("service:getById", async (_event, sessionToken: string, id: number) => {
    try {
      const permissionCheck = await checkPermission(sessionToken, PERMISSIONS.VIEW_SERVICES)
      if (!permissionCheck.success) {
        return { success: false, error: permissionCheck.error }
  ...
  ipcMain.handle("service:getByCategory", async (_event, sessionToken: string, category: string) => {
    try {
      const permissionCheck = await checkPermission(sessionToken, PERMISSIONS.VIEW_SERVICES)
      if (!permissionCheck.success) {
        return { success: false, error: permissionCheck.error }
  ...
  ipcMain.handle("service:update", async (_event, sessionToken: string, data: UpdateServiceDTO) => {
    try {
      const permissionCheck = await checkPermission(sessionToken, PERMISSIONS.MANAGE_SERVICES)
      if (!permissionCheck.success) {
        return { success: false, error: permissionCheck.error }
  ...
  ipcMain.handle("service:delete", async (_event, sessionToken: string, id: number) => {
    try {
      const permissionCheck = await checkPermission(sessionToken, PERMISSIONS.MANAGE_SERVICES)
      if (!permissionCheck.success) {
        return { success: false, error: permissionCheck.error }
