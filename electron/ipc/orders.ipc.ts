...
export function registerOrderHandlers() {
  ipcMain.handle("order:create", async (_event, sessionToken: string, data: CreateOrderDTO) => {
    try {
  ...
  ipcMain.handle("order:getById", async (_event, sessionToken: string, id: number) => {
    try {
  ...
  ipcMain.handle("order:getByNumber", async (_event, sessionToken: string, orderNumber: string) => {
    try {
      const permissionCheck = await checkPermission(sessionToken, PERMISSIONS.VIEW_ORDER)
      if (!permissionCheck.success) {
        return { success: false, error: permissionCheck.error }
  ...
  ipcMain.handle("order:getWithDetails", async (_event, sessionToken: string, id: number) => {
    try {
      const permissionCheck = await checkPermission(sessionToken, PERMISSIONS.VIEW_ORDER)
      if (!permissionCheck.success) {
        return { success: false, error: permissionCheck.error }
  ...
  ipcMain.handle("order:getAll", async (_event, sessionToken: string, limit?: number, offset?: number) => {
    try {
      const permissionCheck = await checkPermission(sessionToken, PERMISSIONS.VIEW_ORDER)
      if (!permissionCheck.success) {
        return { success: false, error: permissionCheck.error }
  ...
  ipcMain.handle("order:getByStatus", async (_event, sessionToken: string, status: OrderStatus) => {
    try {
      const permissionCheck = await checkPermission(sessionToken, PERMISSIONS.VIEW_ORDER)
      if (!permissionCheck.success) {
        return { success: false, error: permissionCheck.error }
  ...
  ipcMain.handle("order:getByDateRange", async (_event, sessionToken: string, startDate: string, endDate: string) => {
    try {
      const permissionCheck = await checkPermission(sessionToken, PERMISSIONS.VIEW_ORDER)
      if (!permissionCheck.success) {
        return { success: false, error: permissionCheck.error }
  ...
  ipcMain.handle("order:search", async (_event, sessionToken: string, query: string) => {
    try {
      const permissionCheck = await checkPermission(sessionToken, PERMISSIONS.VIEW_ORDER)
      if (!permissionCheck.success) {
        return { success: false, error: permissionCheck.error }
  ...
  ipcMain.handle("order:updateStatus", async (_event, sessionToken: string, orderId: number, status: OrderStatus) => {
    try {
  ...
  ipcMain.handle("order:update", async (_event, sessionToken: string, orderId: number, updates: any) => {
    try {
  ...
  ipcMain.handle("order:delete", async (_event, sessionToken: string, id: number) => {
    try {
